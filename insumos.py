...
class Insumos:
    ...
    def consultaPorCodigo(self,codigo):
        ...
        while i<len(sheet.values):
            if str(sheet.values[i][0]) == str(codigo):
                ...
                break
            i += 1
        self.codigo = result[0]
        self.descricao = result[1]
        self.unidadeMedida = result[2]
        self.precoUnitario = self.convertFloat(result[4])
        #return result
    ...
    def consultaPorDescricao(self,descri):
        ...
        cod = [linha[0] for linha in planilha]
        matches = []
        i = 0
        n = 0
